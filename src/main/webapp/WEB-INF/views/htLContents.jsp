...
<div class="wrap">
	...
	<div class="content">
		...
		<h3 class="login-header">숙소 이용 상세내역</h3>
			...
			<c:forEach var="htLitem" items="${htLList}">
			<c:set var="ca" value="${htLitem.ca_num}"></c:set>
			<c:if test="${fn:contains(ca,'H')}">
				
				<tr>
					<th class="hn" scope="row">예약번호</th>
					<td class="hn"><div>${htLitem.ca_num}</div></td>
				</tr>
				<tr>	
					<th scope="row">반려동물 아이디</th>
					<td><div>${htLitem.p_pid}</div></td>
				</tr>
				<tr>	
					<th scope="row">업체 아이디</th>
					<td><div>${htLitem.h_id}</div></td>
				</tr>
				<tr>	
					<th scope="row">숙소 번호</th>
					<td><div>${htLitem.s_num}</div></td>
				</tr>
				<tr>	
					<th scope="row">체크인</th>
					<td><div>${htLitem.ca_in}</div></td>
				</tr>
				<tr>	
					<th scope="row">체크아웃</th>
					<td><div>${htLitem.ca_out}</div></td>
				</tr>
				<tr>	
					<th scope="row">수락여부</th>
					<td><c:set var= "level" value="${htLitem.ca_suc}"/>
