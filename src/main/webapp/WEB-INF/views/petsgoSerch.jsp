		...
		<section>
			<div class="content">
				<div class="board-form">
					...
					<label><input type="button" value="View" onclick="location.href='./viewSearch'"/>조회수순</label> 
					<br><br>
					<div class="data-area">
						<div class="title-row">
						...
						<c:forEach var="hitem" items="${hList}">
							<div class="data-row">
								<div class="t-no p-10">${hitem.bh_num}</div>
								<div class="t-title p-30">
									<a href="hostcontents?bh_num=${hitem.bh_num}">${hitem.bh_title}</a>
								</div>
								<div class="t-name p-15">${hitem.h_id}</div>
								<div class="t-view p-15">${hitem.bh_view}</div>
								<div class="t-like p-10">${hitem.bh_recom}</div>
								<div class="t-date p-15">
									<fmt:formatDate value="${hitem.bh_date}"
										pattern="yyyy-MM-dd"/>
								</div>
							</div>
						</c:forEach>
